...
from geogigpy.diff import Diffentry, ATTRIBUTE_DIFF_MODIFIED
...
class CLIConnector(Connector):
    ...
    def createdat(self):
        dat_path = os.path.join(self.repo.url, ".geogig")
        return datetime.datetime.fromtimestamp(os.stat(dat_path).st_ctime)
    ...
    def isrebasing(self):
        ...
        branchfile = os.path.join(self.repo.url,
                                  '.geogig', 'rebase-apply', 'branch')
    ...
    def checkisrepo(self):
        if not os.path.exists(os.path.join(self.repo.url, '.geogig')):
            msg = "Not a valid GeoGig repository: " + self.repo.url
        ...
        for line in lines:
            ...
            if message:
                ...
                if tokens[0] == 'parent':
                    ...
                    ts_val = (int(tokens[-2]) - int(tokens[-1]))//1000
                    authordate = datetime.datetime.fromtimestamp(ts_val)
                elif tokens[0] == 'committer':
                    committer = tokens[1]
                    ts_val = (int(tokens[-2]) - int(tokens[-1]))//1000
                    committerdate = datetime.datetime.fromtimestamp(ts_val)
                elif tokens[0] == 'message':
                    message = True

        if commitid is not None:
            c = Commit(self.repo, commitid, tree, parents,
                       "\n".join(messagetext), author,
                       authordate, committer, committerdate)
            return c
        else:
            return None
    ...
    def addremote(self, name, url, username, password):
        if username and password:
            commands = ["remote", "add", "-u", username,
                        "--password", password, name, url]
        else:
    ...
    def log(self, tip, sincecommit=None, until=None, since=None, path=None,
            n=None):
        commits = []
    ...
    def conflicts(self):
        ...
        for line in lines:
            if line.startswith("No elements need merging"):
                return {}
            tokens = line.split(" ")
            _conflicts[tokens[0]] = (tokens[1][:40],
                                     tokens[2][:40],
                                     tokens[3][:40])
        return _conflicts
    ...
    def commit(self, message, paths=()):
        commands = ['commit', '-m', '"%s"' % message]
        commands.extend(paths)
        try:
            self.run(commands)
        except GeoGigException as e:
            name_err = "user.name not found"
            mail_err = "user.email not found"
            if name_err in e.args[0] or mail_err in e.args[0]:
    ...
    def diffentryFromString(self, oldcommitref, newcommitref, line):
        ...
        oldref = tokens[-2]
        newref = tokens[-1]
        return Diffentry(self.repo, oldcommitref,
                         newcommitref, oldref, newref, path)
    ...
    def treediff(self, path, refa, refb):
        ...
        # we assume that there are no repeated attrib names with different type
        attribs.update(ftypeb)
    ...
    def exportshp(self, ref, path, shapefile):
        refandpath = ref + ":" + path
        self.run(["shp", "export", refandpath, shapefile,
                  "-o", "--defaulttype"])
    ...
    def valuefromstring(self, value, valuetype):
        ...
        try:
            if valuetype == "BOOLEAN":
                ...
                return float(value)
            elif (valuetype in ["POINT", "LINESTRING", "POLYGON",
    ...
    def featurediff(self, ref, ref2, path):
        ...
        if data is None:
            if data2 is None:
                ...
                return dict(((k, (None, v[0])) for k, v in data2.items()))
        elif data2 is None:
            return dict(((k, (v[0], None)) for k, v in data.items()))
